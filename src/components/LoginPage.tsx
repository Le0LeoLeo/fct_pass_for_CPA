...
import { GraduationCap, MessageCircle, BarChart3, Loader2 } from "lucide-react";
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (error) {
        ...
        setError('Google 登錄失敗，請重試');
        setIsLoading(false);
      } else {
        // 登錄成功，onLogin 會在認證狀態變化時被調用
        // 這裡不需要手動調用，因為 App.tsx 會監聽認證狀態
      }
    } catch (err) {
      console.error('Login error:', err);
      setError('登錄時發生錯誤，請重試');
      setIsLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-blue-50 flex items-center justify-center p-8">
      <div className="w-full max-w-5xl grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
        ...
        >
          ...
          >
            <h2 className="text-[24px] text-gray-900 mb-2">歡迎使用</h2>
            <p className="text-[15px] text-gray-600 mb-8">使用 Google 帳號登入以繼續</p>
            ...
            {error && (
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                className="mb-6 p-4 bg-red-50 border border-red-200 rounded-xl text-red-600 text-[14px]"
              >
                {error}
              </motion.div>
            )}
            ...
            >
              <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
                <Button
                  ...
                  disabled={isLoading}
                  className="w-full h-14 bg-white hover:bg-gray-50 text-gray-700 border-2 border-gray-300 rounded-xl flex items-center justify-center gap-3 shadow-sm hover:shadow-md transition-all"
                >
                  {isLoading ? (
                    <>
                    ...
                    <>
                      ...
                      <span className="text-[16px] font-medium">使用 Google 登入</span>
                    </>
                  )}
                </Button>
              </motion.div>
